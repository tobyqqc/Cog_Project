...
import com.lumosity.utils.StandardDeviationKit;
/**
 * 
 * @author Scott
 *
...
//		GameClass gameClass = GameClass.dao.findById(gameClassId);
		...
		// 获取上次游戏分数
		Integer lastLpi = GameHistory.dao.findLastScoreByUserId(gameId, userId);
		// 获取该游戏的所有分数 

		Newbee newbee = Newbee.dao.findByGameId(gameId);
		Newbee newbee2 = StandardDeviationKit.getSd(newbee, score);

		int LPI = LPIKit.getLPI(lastLpi, score, newbee2.getMean().floatValue(), newbee2.getStd().floatValue());
		c.setAttr("lpi", LPI);
		
		Newbee.dao.update(newbee2);
		...
		/**个人日类型记录表更新*/
		DayClassRecord classRecord = DayClassRecord.dao.findByIdAndClassAndDate2(userId, gameClassId, today);
		if (classRecord != null) {
			//有类型的记录，则覆盖上次数据并记录变化
		...
		/**总LPI(currentLPI)的计算*/
		int currentLPI = LPIKit.getCurrentLPI(userId, today, classLPI, gameClassId);//该日的总LPI
...
//		List<BrainClassProfile> classProfiles = BrainClassProfile.dao.findByUser(userId);
		...
		if (weekRecord != null) {
			if (!(DateTimeKit.format(today).equals(DateTimeKit.format(brainProfile.getLastDate()))))
				weekRecord.set("trainDays", weekRecord.getTrainDays()+1).update();//日期不相同，自增
		...
		if (userGame != null) {
			userGame.set("lastPlayDate", DateTimeKit.getPlayDate()).set("lastPlayScore", score).set("isLike", userGame.getIsLike() == null ? 0 :  userGame.getIsLike())
				.set("playTimes", userGame.getPlayTimes() == null ? 1 : userGame.getPlayTimes()+1).set("lastLevel", level).update();
		} else {
			new UserGame().set("userId", userId).set("gameId", gameId).set("isLocked", 0).set("isLike", 0).set("playTimes", 1)
				.set("lastPlayDate", DateTimeKit.getPlayDate()).set("lastPlayScore", score).set("lastLevel", level).save();
		}
