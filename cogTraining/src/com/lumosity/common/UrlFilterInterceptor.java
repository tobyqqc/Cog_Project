...
public class UrlFilterInterceptor implements Interceptor{
	...
	public String[] urlSkips = new String[] {"/oauth","oauth", "return", "notify","/weixinLogin", "/wxLoginNotify", "wxMpLoginNotify", "/checkMobile", "/wxLoginBind", "/wxPayNotify","/alipayPayNotify",
			"/weixinPCLogin", "/weixinPCLoginNotify", "/checkSignature", "/qqLoginNotify","/qqLoginNotify/login","/qqLoginBindNotify","/wxlogin","/wxlogin/oauth"};
	...
	public void intercept(Invocation inv) {
		...
		for (String url : urlSkips) {
			if (controllerKey.contains(url)) {
				inv.invoke();
				return;
			}
		}
		if (account == null) {
			...
			if ("/index".equals(controllerKey) || "/".equals(controllerKey) || "/register".equals(controllerKey)
				...
				if (/*false&&*/status.getQuestion1Status() == 1 && status.getQuestion2Status() == 1) {
					//完成问卷，跳转主页
					c.redirect("/home");
				} else {
					inv.invoke();
