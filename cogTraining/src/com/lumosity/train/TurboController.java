...
import com.lumosity.games.GameClientInterceptor;
...
public class TurboController extends Controller {
	...
	public boolean isLock(Long userId, Integer gameId) {
		...
		planGames = PlanGame.dao.findByTrainPlanId(trainPlan.getTrainPlanId());
		...
		return (gameClassExpireTime <= System.currentTimeMillis()) && (!gameIds.contains(gameId));
	}
	...
	public void start() {
		...
		if (isLock(userId, planGame.getGameId())/*||true*/) {
			render("/train/turbo_lock.html");
			return;
		}
		
		if (planGame.getIsPlay() == 0) {
			List<PlanGame> planGames = PlanGame.dao.findRemaining(trainPlanId);// 获取用户今日剩余游戏计划(以sequence升序排列)
			if (planGames != null && planGames.size() > 0) {
				planGame = planGames.get(0);
				sequence = planGame.getSequence();
			} else {
				redirect("/home");
				return;
			}
	...
	public void play() {
		...
		if (isLock(userId, planGame.getGameId())) {
			render("/train/turbo_lock.html");
		}
		...
		// 默认用户没有玩过游戏
		String jsonStr = "{\"fit_test\":\"1\",\"game_id\":\"" + gameId + "\",\"game_param\":\"" + gameName
		...
		if (userGame != null) {
			if (userGame.getLastPlayDate() != null && userGame.getLastPlayScore() != null) {
				// 用户玩过游戏,fit_test:0
				jsonStr = "{\"fit_test\":\"0\",\"game_id\":\"" + gameId + "\",\"game_param\":\"" + gameName
						+ "\",\"username\":\"" + userName + "\",\"token\":\""
						+ "uqiwejhbaskjdbasdioqw\",\"game_user_setting\":{\"level\":\""+ (userGame.getLastLevel()==null ? 0 : userGame.getLastLevel()) +"\",\"time\":\"5\",\"score\":\""
						+ userGame.getLastPlayScore() + "\"},\"updated_at\":\"" + userGame.getLastPlayDate() + "\"}";
			}
		}
	...
	public void replay() {
		...
		PlanGame planGame = PlanGame.dao.findByTrainPlanIdAndSequence(trainPlanId, sequence);
		// if (planGame.getIsPlay() == 0) {
		// List<PlanGame> planGames =
		// PlanGame.dao.findRemaining(trainPlanId);//获取用户今日剩余游戏计划(以sequence升序排列)
		// if (planGames != null && planGames.size() > 0) {
		// planGame = planGames.get(0);
		// sequence = planGame.getSequence();
		// }
		...
		setAttr("planGame", planGame);
		setAttr("game", game);
		render("/train/turbo_replay.html");
	...
	public void complete() {
		...
		int LPI = getAttr("lpi");
		/* 保存本次游戏数据 */
		new GameHistory().set("gameId", gameId).set("userId", userId).set("score", score)
				.set("playDate", DateTimeKit.getDate()).set("gameLPI", LPI)
				.set("gameData", jsonObject.getString("trial_data")).save();
	...
	public void recomplete() {
		...
		TrainPlan trainPlan = TrainPlan.dao.findByIdAndDate(userId, DateTimeKit.getDate());// 获取用户今日训练计划
		Integer trainPlanId = trainPlan.getTrainPlanId();// 获取用户今日训练计划id
		PlanGame planGame = PlanGame.dao.findByTrainAndGame(trainPlanId, this.getParaToInt("gameId"));// 获取用户今日剩余游戏计划(以sequence升序排列)

		/* 获取本次游戏的相关数据 */
		int sequence = planGame.getSequence();// 本次游戏所在训练计划中的序列号
		int gameId = planGame.getGameId();
		...
		// planGames.get(0).set("isPlay", 1).update();//设置该游戏状态为已完成
		// trainPlan.set("playTimes",
		// trainPlan.getPlayTimes()+1).update();//设置今日计划的完成状态
