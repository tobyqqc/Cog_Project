...
//		Long userId = account.getUserId();
		...
		for (GameClass gameClass : gameClasses) {
			int num = 0;
			Date startDate = Date.valueOf(fourWeeks.get(0));
//			for (String day : fourWeeks) {
//				DayClassRecord record = DayClassRecord.dao.findByIdAndClassAndDate(
//											userId, gameClass.getGameClassId(), startDate, endDate);
//				if (record !=null) {
//					//有该日该类型的记录
//					num += record.getGameTimes();
//				}
//			}
			Record record = DayClassRecord.dao.findByIdAndClassAndDate(userId, gameClass.getGameClassId(), startDate, endDate);
			if (record !=null) {
				//有该日该类型的记录
				num += record.getBigDecimal("gameTimes").intValue();
			}
		...
		Map<String, Integer> changePie = new LinkedHashMap<>();
		for (GameClass gameClass : gameClasses) {
...
//		Long userId = account.getUserId();
		...
		for (String day : days) {
			if (DateTimeKit.isWeekend(day)) {
				...
				if (weekRecord != null) {
					...
					if (weekRecord.getLPI() == 0) 
						continue;
					weekLPI.add(weekRecord.getLPI());
					lastLpi = weekRecord.getLPI();
		...
		if (brainProfile != null) {
			...
			startLPI = DayRecord.dao.findNomalById(userId).getLPI();
